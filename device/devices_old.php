...
<?php
    ...
    if ($data->error_code == 1) {
        ...
        for ($i = $startNum; $i < $totalRows; $i++) {
            $strIsActivate = ($data->device_infos[$i]->device_status->is_activate == 1) ? "온라인" : "오프라인";
            if (!$noSearch) {
                ...
                $is_activate = (strpos(strtolower((String)$strIsActivate), $searchStr) !== FALSE) ? TRUE : FALSE;
            }

            if ( $group_name || $name || $model_name || $ip_addr || $tcp_port || $web_port || $mac_addr || $serial_no || $is_activate || $noSearch) {
                $str .= '<tr class="">';
                ...
                $str .= '<td class="py-2">'.$strIsActivate.'</td>';
                $str .= '<td class="py-2">';
        ...
        if (!$noSearch) {
            ...
            for ($i = 0; $i < $totalRows; $i++) {
                ...
                if ( $group_name || $name || $model_name || $ip_addr || $mac_addr || $serial_no || $tcp_port || $web_port || $is_activate ) {
                    $searchRows++;
                }
            }
    ...
    $deviceGroupsDataRs = str_replace('{"d_group_infos":[','[',$deviceGroupsDataRs);
    $deviceGroupsDataRs = str_replace(',"error_code":1,"error_msg":"success","error_msg_detail":"","work_info":{"client_work_id":"","method":"GET","path":"/v1/device-groups","work_id":0}}','',$deviceGroupsDataRs);
...
<style>
    ...
    .modal-content::-webkit-scrollbar-thumb {background-color: #cccccc;border-radius: 15px;}
    .modal-content::-webkit-scrollbar-track {background-color: #ececec;border-radius: 15px;}
    ...
    .deviceScanForm:hover{background-color: #f3f3f3; cursor: pointer;}
        ...
        <div class="py-4 px-3 px-md-4">
            <div id="sub-sidebar">
                <div class="card mb-3 mb-md-4 sub-sidebar-menu">
                    ...
                    <div>
                        <button type="button" class="btn" style="color: #fff; background-color: #265df1; border-color: #265df1; float: right; margin-left: 5px;" onclick="javascript: deviceGroupSet('add',0);" data-toggle="modal" data-target="#deviceGroupModal" data-dismiss="modal" data-backdrop="static" keybord="false">추가</button>
                        <button type="button" class="btn" onclick="javascript: device_group_del();" style="color: #fff; background-color: #ff6500; border-color: #ff6500; float: right;">삭제</button>
                    </div>
                </div>
            </div>
            ...
            <div class="card mb-3 mb-md-4 display-content" id="type-content" style="height: 55rem;">
                        ...
                        <form name = "search_form" method = "GET">
                            ...
                            <div class="form-row">
                                <div class="form-group col-4 col-md-2">
                                    <label for="devices_id">Search</label>
                                ...
                                <div class="form-group col-4 col-md-10">
                                    <button type="button" onclick = "javascript: goSearch('devices');" class="btn" style="position: relative; top: 32px; color: #fff; background-color: #ffc300; border-color: #ffc300;">검색</button>
                                    <button type="button" class="btn" onclick="javascript: deviceInsertReset();" style="position: relative; top: 32px; color: #fff; background-color: #265df1; border-color: #265df1;" data-toggle="modal" data-target="#deviceInsertModal" data-dismiss="modal" data-backdrop="static" keybord="false">추가</button>
                                    <button type="button" onclick = "javascript: goDelete('more','devices');" class="btn" style="position: relative; top: 32px; color: #fff; background-color: #ff6500; border-color: #ff6500;">삭제</button>
                                </div>
                            </div>
                ...
                <div class="card-footer d-block d-md-flex align-items-center d-print-none" style="border-top: 1px solid #d8d8d8;">
                    ...
                    <div class="d-flex mb-2 mb-md-0 ml-3" style="color: #8a8a8a; float: left; cursor: pointer;" onclick="javascript: goPage('1','devices');">처음으로</div>
                    ...
                    <div class="d-flex mb-2 mb-md-0 ml-3" style="float: left;"><button type="button" class="btn" style="color: #000; background-color: #f7f7f7; border-color: #cccccc;" onclick="javascript: goPage('search_page','devices');">이동</button></div>
...
<div id="deviceInsertModal" class="modal fade" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog rounded" role="document" style="top: 6rem; padding: 0!important; width: 85rem; left: 7.5rem;">
        <div class="modal-content" style="padding: 10px 0!important; overflow: auto; height: 50rem;">
            <header class="modal-header flex-column justify-content-center" style="border-bottom: 1px solid #a7a7a7; padding: 1rem 2rem 0 1.7rem;">
                <div style="width: 100%;">
                    <h4>단말기 추가
                    ...
                    <div class="card-body">
                        <div class="form-group col-4 col-md-12">
                            <div class="card-title">&nbsp;
                                <button type="button" onclick="javascript: deviceSearch();" class="btn" style="float: right; color: #fff; background-color: #ffc300; border-color: #ffc300;">검색</button>
                                <span style="color: #8a8a8a; float: right; margin: 0.5rem 2rem 0 0; font-size: 20px;" id="device-scan-total-row"></span>
                            </div>
                        </div>
                        <!-- device-scan -->
                        ...
                        <form name="device_insert_form" class="deviceFrom" method="POST">
                            <div style="width: 50%; float: left;">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control input" name="group_id" id="deviceInsertGroupId" style="width: 57%;">
                                        <?=$deviceGroupsStr?>
                                    </select>
                                    <button type="button" class="btn" onclick="javascript: deviceGroupSet('addBtn',0);" data-toggle="modal" data-target="#deviceGroupModal" data-backdrop="static" keybord="false" href="#" style="position: relative; top: -2.2px; color: #fff; background-color: #265df1; border-color: #265df1;">추가</button>
                                </div>
                            ...
                            <div style="width: 50%; float: right;">
                                <div id="device_insert_sync_form">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control input" name="model_name">
                                    </div>
                                    <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <label for="device_insert_time_sync">연결시 시간 동기화하기</label>
                                </div>
                            </div>
							
							<button type="button" onclick="javascript: go_device_insert();" class="btn btn-primary btn-block">저 장</button>
						</form>
					</div>
				</div>
            </div>
...
<div id="deviceUpdateModal" class="modal fade" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog rounded py-5" role="document" style="top: 10rem; padding: 0!important; width: 65rem;">
        <div class="modal-content" style="padding: 10px 0!important; overflow: auto; height: 39rem;">
            <header class="modal-header flex-column justify-content-center" style="border-bottom: 1px solid #a7a7a7; padding: 1rem 2rem 0 1.7rem;">
                <div style="width: 100%;">
                    <h4>단말기 수정
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close" style="padding: 1.2rem 1rem 0 0;"><span aria-hidden="true">&times;</span></button>
                    ...
                    <div class="card-body">
                        <form name="device_update_form" method = "POST">
                            <div style="width: 50%; float: left;">
                                <div class="form-group">
                                    <input class="form-control input_label" value="단말기 그룹" disabled>
                                    <select class="form-control input" id="deviceUpdateGroupId" name="group_id">
                                        <?=$deviceGroupsStr?>
                                    </select>
                                </div>
                                <div class="form-group">
                            ...
                            <div style="width: 50%; float: right;">
                                ...
                                <div id="device_update_sync_form">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <label for="device_update_time_sync">연결시 시간 동기화하기</label>
                                    </div>
                                </div>
                            </div>
							
							<button type="button" id="deviceUpdateBtn" onclick="javascript: go_device_update();" class="btn btn-primary btn-block">저 장</button>
						</form>
					</div>
				</div>
            </div>
...
<script>
    ...
    // Data
    var deviceGroupsDataRs = '<?echo $deviceGroupsDataRs?>';
    ...
    $(document).ready(function(){
        ...
        for (let i = 0; i < $("#ztree .node_name").length; i++) {
            var deviceGroup = zNodes.filter(function (e) {
