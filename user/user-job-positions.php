...
<?php
    ...
    $dataRs = get("/v1/job-positions");
    $data = json_decode($dataRs);

    if ($data->error_code == 1) {
        $str = "";
        $searchRows = 0;
        $totalRows = count($data->du_job_position_infos);
        $startNum = ($search_start_num == 0 ) ?  ($page_no-1) * $page_count : $search_start_num;

        for ($i = $startNum; $i < $totalRows; $i++) {
            if (!$noSearch) {
                $jobPositionName = (strpos((String)$data->du_job_position_infos[$i]->name, $searchStr) !== FALSE) ? TRUE : FALSE;
            }

            if ( $jobPositionName || $noSearch) {
                $str .= '<tr class="">';
                $str .= '<td class="py-2"><input type="checkbox" name="choice[]" value="'.$data->du_job_position_infos[$i]->id.'"></td>';
                $str .= ($noSearch) ? '<td class="py-2">'.($i+1).'</td>' : '<td class="py-2">'.( $searchRows+(($page_no-1)*$page_count) + 1 ).'</td>';
                $str .= '<td class="py-2">'.$data->du_job_position_infos[$i]->name.'</td>';
                $str .= '<td class="py-2">';
                $str .= '<div class="position-relative">';
                $str .= '<a class="link-dark d-inline-block" onclick="javascript: user_job_position_update_info('.$data->du_job_position_infos[$i]->id.');" data-toggle="modal" data-target="#userJopPositionModal" href="#">';
                $str .= '<i class="gd-pencil icon-text"></i>';
                $str .= '</a>';
                $str .= '<a class="link-dark d-inline-block" onclick="javascript: goDelete('.$data->du_job_position_infos[$i]->id.',\'user-job-position\');" href="#">';
        ...
        for ($i = 1; $i <= $totalPage; $i++) {
            ...
            $strPage .= '<a id="datatablePagination'.$i.'" class="page-link '.$active.'" onclick="javascript: goPage('.$i.',\'user-job-position\');" href="#" data-dt-page-to="'.$i.'">'.$i.'</a>';
            $strPage .= '</li>';
        }
    }
?>
        ...
        <div class="py-4 px-3 px-md-4">
            ...
            <div class="card mb-3 mb-md-4 display-content" id="type-content" style="height: 55rem;">
                <div class="card-body">
                    ...
                    <nav class="d-none d-md-block" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">사용자 직급</li>
                        </ol>
                    </nav>
                    <!-- End Breadcrumb -->

                    <div class="mb-3 mb-md-4 d-flex justify-content-between">
                        <div class="h3 mb-0">사용자 직급</div>
                        <div style="padding-right: 5rem;"><i onclick="javascript: refresh('user-job-positions');" class = "gd-reload h4" style = "cursor : pointer;"></i></div>
                    </div>
                        ...
                        <form name = "search_form" method = "GET">
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-8 col-md-10">
                                    <button type="button" onclick = "javascript: goSearch('user-job-positions');" class="btn" style="position: relative; top: 32px; color: #fff; background-color: #ffc300; border-color: #ffc300;">검색</button>
                                    <button type="button" class="btn" onclick="job_position_form_reset();" style="position: relative; top: 32px; color: #fff; background-color: #265df1; border-color: #265df1;" data-toggle="modal" data-target="#userJopPositionModal">추가</button>
                                    <button type="button" onclick = "javascript: goDelete('more','user-job-positions');" class="btn" style="position: relative; top: 32px; color: #fff; background-color: #ff6500; border-color: #ff6500;">삭제</button>
                    ...
                    <div style="height: 35rem; overflow: auto;">
                        <form name = "contents_show_form">
                            <table class="table text-nowrap mb-0">
                                ...
                                <tbody id="user-job-positions-content">
                                    <?=$str?>
                                </tbody>
                            </table>
                ...
                <div class="card-footer d-block d-md-flex align-items-center d-print-none" style="border-top: 1px solid #d8d8d8;">
                    <nav class="d-flex d-print-none" aria-label="Pagination">
                        <ul class="pagination justify-content-end font-weight-semi-bold mb-0">
                            <li class="page-item" id="page-prev">
                                <button type = "button" id="datatablePaginationPrev" class="page-link" onclick="javascript: goPage('<?=$page_no-1?>','user-job-positions');" aria-label="Previous">
                                    <i class="gd-angle-left icon-text icon-text-xs d-inline-block"></i>
                                </button>
                            </li>
                                <?=$strPage?>
                            <li class="page-item" id="page-next">
                                    <button type = "button" id="datatablePaginationNext" class="page-link" onclick="javascript: goPage('<?=$page_no+1?>','user-job-positions');" aria-label="Next">
                                    <i class="gd-angle-right icon-text icon-text-xs d-inline-block"></i>
                                </button>
                            </li>
                        </ul>
                    </nav>
                    <div class="d-flex mb-2 mb-md-0 ml-3" style="color: #8a8a8a; float: left; cursor: pointer;" onclick="javascript: goPage('1','user-job-positions');">처음으로</div>
                    ...
                    <div class="d-flex mb-2 mb-md-0 ml-3" style="float: left;"><button type="button" class="btn" style="color: #000; background-color: #f7f7f7; border-color: #cccccc;" onclick="javascript: goPage('search_page','user-job-positions');">이동</button></div>
                    ...
                    <div class="d-flex mb-2 mb-md-0 ml-3" style="float: left;">
                        <select id="page_count_change" onchange="javascript: changeCount('user-job-positions');" style="border:1px solid #cccccc; color: #8a8a8a; height: 44px;">
...
<!-- User Insert Modal -->
<div id="userJopPositionModal" class="modal fade" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog rounded py-5" role="document">
        <div class="modal-content py-5">
            ...
            <div class="modal-body pt-3 mb-5 mb-md-7">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">사용자 직급 추가</h4>
						<form name = "job_positions_form" method = "POST">
							...
							<button type="button" id="jobPositionInsertBtn" onclick="javascript: go_user_job_position_insert();" class="btn btn-primary btn-block">생 성</button>
						</form>
					</div>
				</div>
            </div>
...
<script>
    function job_position_form_reset() {
        var f = $("form[name='job_positions_form']").find("input[class=form-control]");
        for (let i = 0; i < f.length; i++) {
            f[i].value="";
        }
        $("#userJopPositionModal .card-title").text("사용자 직급 추가");
        $("#jobPositionUpdateBtn").remove();
        $("#jobPositionInsertBtn").remove();
        var btn = '<button type="button" id="jobPositionInsertBtn" onclick="javascript: go_user_job_position_insert();" class="btn btn-primary btn-block">생 성</button>';
