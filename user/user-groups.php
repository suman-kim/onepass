...
<?php
    ...
    $dataRs = get("/v1/user-groups");
    $data = json_decode($dataRs);

    if ($data->error_code == 1) {
        $str = "";
        $searchRows = 0;
        $totalRows = count($data->du_group_infos);
        $startNum = ($search_start_num == 0 ) ?  ($page_no-1) * $page_count : $search_start_num;

        for ($i = $startNum; $i < $totalRows; $i++) {
            $userGroupRs = getParam("/v1/user-groups", $data->du_group_infos[$i]->parent_id);
            $userGroup = json_decode($userGroupRs);
            $userParentName[$i] = $userGroup->du_group_info->name;

            if (!$noSearch) {
                $groupName = (strpos((String)$data->du_group_infos[$i]->name, $searchStr) !== FALSE) ? TRUE : FALSE;
                $groupParentName = (strpos((String)$userParentName[$i], $searchStr) !== FALSE) ? TRUE : FALSE;
            }

            if ( $groupName || $groupParentName || $noSearch) {
                $str .= '<tr class="">';
                $str .= '<td class="py-2"><input type="checkbox" name="choice[]" value="'.$data->du_group_infos[$i]->id.'"></td>';
                $str .= ($noSearch) ? '<td class="py-2">'.($i+1).'</td>' : '<td class="py-2">'.( $searchRows+(($page_no-1)*$page_count) + 1 ).'</td>';
                $str .= '<td class="py-2">'.$data->du_group_infos[$i]->name.'</td>';
                $str .= '<td class="align-middle py-2">'.$userParentName[$i].'</td>';
                $str .= '<td class="py-2">';
                $str .= '<div class="position-relative">';
                $str .= '<a class="link-dark d-inline-block" onclick="javascript: user_group_update_info('.$data->du_group_infos[$i]->id.');" data-toggle="modal" data-target="#userGroupModal" href="#">';
                $str .= '<i class="gd-pencil icon-text"></i>';
                $str .= '</a>';
                $str .= '<a class="link-dark d-inline-block" onclick="javascript: goDelete('.$data->du_group_infos[$i]->id.',\'user-groups\');" href="#">';
        ...
        for ($i = 1; $i <= $totalPage; $i++) {
            ...
            $strPage .= '<a id="datatablePagination'.$i.'" class="page-link '.$active.'" onclick="javascript: goPage('.$i.',\'user-groups\');" href="#" data-dt-page-to="'.$i.'">'.$i.'</a>';
            $strPage .= '</li>';
        }
    }
?>
            ...
            <div class="card mb-3 mb-md-4 display-content" id="type-content" style="height: 55rem;">
                <div class="card-body">
                    ...
                    <div class="mb-3 mb-md-4 d-flex justify-content-between">
                        <div class="h3 mb-0">사용자 그룹</div>
                        <div style="padding-right: 5rem;"><i onclick="javascript: refresh('user-groups');" class = "gd-reload h4" style = "cursor : pointer;"></i></div>
                    </div>
                        ...
                        <form name = "search_form" method = "GET">
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-4 col-md-10">
                                    <button type="button" onclick = "javascript: goSearch('user-groups');" class="btn" style="position: relative; top: 32px; color: #fff; background-color: #ffc300; border-color: #ffc300;">검색</button>
                                    <button type="button" class="btn" onclick="group_form_reset();" style="position: relative; top: 32px; color: #fff; background-color: #265df1; border-color: #265df1;" data-toggle="modal" data-target="#userGroupModal">추가</button>
                                    <button type="button" onclick = "javascript: goDelete('more','user-groups');" class="btn" style="position: relative; top: 32px; color: #fff; background-color: #ff6500; border-color: #ff6500;">삭제</button>
                            ...
                            <table class="table text-nowrap mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="font-weight-semi-bold border-top-0 py-2">부모 그룹</th>
                ...
                <div class="card-footer d-block d-md-flex align-items-center d-print-none" style="border-top: 1px solid #d8d8d8;">
                    <nav class="d-flex d-print-none" aria-label="Pagination">
                        <ul class="pagination justify-content-end font-weight-semi-bold mb-0">
                            <li class="page-item" id="page-prev">
                                <button type = "button" id="datatablePaginationPrev" class="page-link" onclick="javascript: goPage('<?=$page_no-1?>','user-groups');" aria-label="Previous">
                                    <i class="gd-angle-left icon-text icon-text-xs d-inline-block"></i>
                                </button>
                            </li>
                                <?=$strPage?>
                            <li class="page-item" id="page-next">
                                    <button type = "button" id="datatablePaginationNext" class="page-link" onclick="javascript: goPage('<?=$page_no+1?>','user-groups');" aria-label="Next">
                                    <i class="gd-angle-right icon-text icon-text-xs d-inline-block"></i>
                                </button>
                            </li>
                        </ul>
                    </nav>
                    <div class="d-flex mb-2 mb-md-0 ml-3" style="color: #8a8a8a; float: left; cursor: pointer;" onclick="javascript: goPage('1','user-groups');">처음으로</div>
                    ...
                    <div class="d-flex mb-2 mb-md-0 ml-3" style="float: left;"><button type="button" class="btn" style="color: #000; background-color: #f7f7f7; border-color: #cccccc;" onclick="javascript: goPage('search_page','user-groups');">이동</button></div>
                    ...
                    <div class="d-flex mb-2 mb-md-0 ml-3" style="float: left;">
                        <select id="page_count_change" onchange="javascript: changeCount('user-groups');" style="border:1px solid #cccccc; color: #8a8a8a; height: 44px;">
...
<?php
$parent_groups = "";
$cnt = count($data->du_group_infos);
for ($i=0; $i < $cnt; $i++) { 
    $parent_groups .= "<option value='".$data->du_group_infos[$i]->id."'>".$data->du_group_infos[$i]->name."</option>";
}
?>
            ...
            <div class="modal-body pt-3 mb-5 mb-md-7">
                <div class="card">
                    <div class="card-body">
						...
						<form name = "user_group_form" method = "POST">
							...
							<button type="button" id="userGroupInsertBtn" onclick="javascript: go_user_group_insert();" class="btn btn-primary btn-block">생 성</button>
						</form>
					</div>
				</div>
            </div>
...
<script>
    function group_form_reset() {
        var f = $("form[name='user_group_form']").find("input[class=form-control]");
        for (let i = 0; i < f.length; i++) {
            f[i].value="";
        }
        $("#userGroupModal .card-title").text("사용자 그룹 추가");
        $("#userGroupUpdateBtn").remove();
        $("#userGroupInsertBtn").remove();
        var btn = '<button type="button" id="userGroupInsertBtn" onclick="javascript: go_user_group_insert();" class="btn btn-primary btn-block">생 성</button>';
