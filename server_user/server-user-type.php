...
<?php
    ...
    $dataRs = get("/v1/server/user-types");
    $data = json_decode($dataRs);
    
    if ($data->error_code == 1) {
        $str = "";
        $searchRows = 0;
        $totalRows = count($data->type_infos);
        $startNum = ($search_start_num == 0 ) ?  ($page_no-1) * $page_count : $search_start_num;

        for ($i = $startNum; $i < $totalRows; $i++) {
            if (!$noSearch) {
                $typeName = (strpos((String)$data->type_infos[$i]->name, $searchStr) !== FALSE) ? TRUE : FALSE;
            }

            if ( $typeName || $noSearch) {
                $str .= '<tr class="">';
                $str .= '<td class="py-2"><input type="checkbox" name="choice[]" value="'.$data->type_infos[$i]->id.'"></td>';
                $str .= ($noSearch) ? '<td class="py-2">'.($i+1).'</td>' : '<td class="py-2">'.( $searchRows+(($page_no-1)*$page_count) + 1 ).'</td>';
                $str .= '<td class="py-2">'.$data->type_infos[$i]->name.'</td>';
                $str .= '<td class="py-2">';
                $str .= '<div class="position-relative">';
                $str .= '<a class="link-dark d-inline-block" onclick="javascript: server_user_type_update_info('.$data->type_infos[$i]->id.');" data-toggle="modal" data-target="#serverUserTypeModal" href="#">';
                $str .= '<i class="gd-pencil icon-text"></i>';
                $str .= '</a>';
                $str .= '<a class="link-dark d-inline-block" onclick="javascript: goDelete('.$data->type_infos[$i]->id.',\'server-user-type\');" href="#">';
        ...
        if (!$noSearch) {
            ...
            for ($i = 0; $i < $totalRows; $i++) {
                $typeName = (strpos((String)$data->type_infos[$i]->name, $searchStr) !== FALSE) ? TRUE : FALSE;
                $typeLevel = (strpos((String)$data->type_infos[$i]->level, $searchStr) !== FALSE) ? TRUE : FALSE;

                if ( $typeName || $typeLevel ) {
                    $searchRows++;
                }
            }
        ...
        for ($i = 1; $i <= $totalPage; $i++) {
            ...
            $strPage .= '<a id="datatablePagination'.$i.'" class="page-link '.$active.'" onclick="javascript: goPage('.$i.',\'server-user-type\');" href="#" data-dt-page-to="'.$i.'">'.$i.'</a>';
            $strPage .= '</li>';
        }
    }else {
        ...
        $str .= '<div>';
        $str .= '<span >사용자 유형을 찾지 못했습니다.</span>';
        $str .= '</div>';
        $str .= '</td>';
        $str .= '</tr>';
        
        $strPage = '<li class="page-item d-none d-md-block">';
        $strPage .= '<a id="datatablePagination1" class="page-link active" onclick="javascript: goPage(1,\'server-user-type\');" href="#" data-dt-page-to="1">1</a>';
        $strPage .= '</li>';
    }
?>
        ...
        <div class="py-4 px-3 px-md-4">
            ...
            <div class="card mb-3 mb-md-4 display-content" id="type-content" style="height: 55rem;">
                <div class="card-body">
                    ...
                    <nav class="d-none d-md-block" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">서버 사용자 유형</li>
                        </ol>
                    </nav>
                    <!-- End Breadcrumb -->

                    <div class="mb-3 mb-md-4 d-flex justify-content-between">
                        <div class="h3 mb-0">서버 사용자 유형</div>
                        <div style="padding-right: 5rem;"><i onclick="javascript: refresh('server-user-type');" class = "gd-reload h4" style = "cursor : pointer;"></i></div>
                    </div>
                        ...
                        <form name = "search_form" method = "GET">
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-8 col-md-10">
                                    <button type="button" onclick = "javascript: goSearch('server-user-type');" class="btn" style="position: relative; top: 32px; color: #fff; background-color: #ffc300; border-color: #ffc300;">검색</button>
                                    <button type="button" class="btn" onclick="server_user_type_form_reset();" style="position: relative; top: 32px; color: #fff; background-color: #265df1; border-color: #265df1;" data-toggle="modal" data-target="#serverUserTypeModal">추가</button>
                                    <button type="button" onclick = "javascript: goDelete('more','server-user-type');" class="btn" style="position: relative; top: 32px; color: #fff; background-color: #ff6500; border-color: #ff6500;">삭제</button>
                                </div>
                            </div>
                ...
                <div class="card-footer d-block d-md-flex align-items-center d-print-none" style="border-top: 1px solid #d8d8d8;">
                    <nav class="d-flex d-print-none" aria-label="Pagination">
                        <ul class="pagination justify-content-end font-weight-semi-bold mb-0">
                            <li class="page-item" id="page-prev">
                                <button type = "button" id="datatablePaginationPrev" class="page-link" onclick="javascript: goPage('<?=$page_no-1?>','server-user-type');" aria-label="Previous">
                                    <i class="gd-angle-left icon-text icon-text-xs d-inline-block"></i>
                                </button>
                            </li>
                                <?=$strPage?>
                            <li class="page-item" id="page-next">
                                    <button type = "button" id="datatablePaginationNext" class="page-link" onclick="javascript: goPage('<?=$page_no+1?>','server-user-type');" aria-label="Next">
                                    <i class="gd-angle-right icon-text icon-text-xs d-inline-block"></i>
                                </button>
                            </li>
                        </ul>
                    </nav>
                    <div class="d-flex mb-2 mb-md-0 ml-3" style="color: #8a8a8a; float: left; cursor: pointer;" onclick="javascript: goPage('1','server-user-type');">처음으로</div>
                    ...
                    <div class="d-flex mb-2 mb-md-0 ml-3" style="float: left;"><button type="button" class="btn" style="color: #000; background-color: #f7f7f7; border-color: #cccccc;" onclick="javascript: goPage('search_page','server-user-type');">이동</button></div>
                    ...
                    <div class="d-flex mb-2 mb-md-0 ml-3" style="float: left;">
                        <select id="page_count_change" onchange="javascript: changeCount('server-user-type');" style="border:1px solid #cccccc; color: #8a8a8a; height: 44px;">
...
<!-- User Insert Modal -->
<div id="serverUserTypeModal" class="modal fade" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            <div class="modal-body pt-3 mb-5 mb-md-7">
                <div class="card">
                    <div class="card-body">
						...
						<form name = "server_user_type_form" method = "POST">
							<div class="form-group">
								<label for="type_name">서버 사용자 유형 이름</label>
								<input type="text" class="form-control" id="type_name" name="type_name">
							</div>
							<button type="button" id="serverUserTypeInsertBtn" onclick="javascript: go_server_user_type_insert();" class="btn btn-primary btn-block">생 성</button>
						</form>
					</div>
				</div>
            </div>
...
<script>
    function server_user_type_form_reset() {
        var f = $("form[name='server_user_type_form']").find("input[class=form-control]");
        for (let i = 0; i < f.length; i++) {
            f[i].value="";
        }
        $("#serverUserTypeModal .card-title").text("서버 사용자 유형 추가");
        $("#serverUserTypeUpdateBtn").remove();
        $("#serverUserTypeInsertBtn").remove();
        var btn = '<button type="button" id="serverUserTypeInsertBtn" onclick="javascript: go_server_user_type_insert();" class="btn btn-primary btn-block">생 성</button>';
